...
@section('content')
<form action="{{route('administrator.category.edit', ['id' => $category->id])}}" method="post">
@csrf
        ...
        <div class="card-body">
           <div class="tab-content mt-3">
            <div class="tab-pane active" id="tresc" role="tabpanel">
                <textarea class="form-control" name="description" rows="3" style="height:350px;"><?=$category->description;?></textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="col-3">
            ...
            <div class="control-group">
                ...
                <select class="custom-select custom-select-sm flex-column" name="published" id="stan">
                    <option {{ ($category->published == 1) ? 'selected' : ''}} value="1">Opublikowano</option>
                    <option {{ ($category->published == 0) ? 'selected' : ''}} value="0">Nie opublikowano</option>
                    <option {{ ($category->published == 2) ? 'selected' : ''}} value="2">Zarchowizowano</option>
                    <option {{ ($category->published == -2) ? 'selected' : ''}} value="-2">W koszu</option>
                </select>
            </div>
        </div>
    </div>
