...
use App\Events\ImageIsUploading;
...
class LfmPath
{
    ...
    public function folders()
    {
        ...
        $pattern = request()->input('search_text');
        if ($pattern) {
            $directories = array_filter($directories, function ($directory) {
                return !str_contains($directory, 'thumbs');
            });
            $directories = array_filter($directories, function ($directory) use ($pattern) {
    ...
    public function files()
    {
        ...
        if ($pattern) {
            ...
            $files =  array_filter($files, function ($filepath) use ($pattern) {
    ...
    {
        // dd($this->helper->getNameFromPath($item_path), $isDirectory);
